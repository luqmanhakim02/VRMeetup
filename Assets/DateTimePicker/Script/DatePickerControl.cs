...
public class DatePickerControl : MonoBehaviour {
    ...
    public TMPro.TMP_Text targetDateTimeField;
	...
	[HideInInspector]
	public string formatoCustom="dddd dd/MM/yyyy HH:mm";
	[HideInInspector]
    ...
    public void Confirm()
    {
        string formatted = DateGlobal.ToString("yyyy-MM-ddTHH:mm");

        if (targetDateTimeField != null)
            targetDateTimeField.text = formatted;

        if (pickerPanel != null)
            pickerPanel.SetActive(false); // Auto-close

        Debug.Log("Confirmed and closed: " + formatted);
    }
